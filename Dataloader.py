...
def _get_centroid(mask: np.ndarray) -> List[np.int32]:
    ...
    row_mean, col_mean = coords[:, 0].mean(), coords[:, 1].mean()
    return [np.int32(round(row_mean)), np.int32(round(col_mean))]
...
def _get_bbox(mask: np.ndarray) -> List[np.int32]:
    ...
    return [
        np.int32(row_min),
        np.int32(col_min),
        np.int32(col_max - col_min + 1),
        np.int32(row_max - row_min + 1),
    ]


def _get_depth(depth_path: Path, centroid_row: np.int32, centroid_col: np.int32) -> np.float32:
    depth = cv2.imread(str(depth_path), cv2.IMREAD_UNCHANGED)
    if depth is None:
        raise FileNotFoundError(f"Depth image not found: {depth_path}")

    h, w = depth.shape[:2]
    row = int(np.clip(int(centroid_row), 0, h - 1))
    col = int(np.clip(int(centroid_col), 0, w - 1))

    return np.float32(depth[row, col])
